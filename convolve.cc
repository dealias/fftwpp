...
//    if(S == 1) { // FIXME
      if(p == 1) {
        ...
        Complex *K=ComplexAlign(Cm*(p/2+1));
        double *R=(double *) K;
...
void fftPadReal::backwardInner(Complex *F0, Complex *f, size_t r0, Complex *W)
{
  ...
  if(r0 == 0) {
    ...
    */
    ...
    Ft=W;
    Ftm=W+m*p2;
    Complex Zetar1=Zetaqr[1];
    PARALLELIF(
      m > threshold,
      for(size_t s=0; s < m; ++s) {
        f2t[s] += real(Ft[s]);
        f2t1[s] += realproduct(Zetar1,Ftm[s]);
      });
    PARALLELIF(
      (p2-1)*m > threshold,
      for(size_t t=1; t < p2; ++t) {
        ...
        size_t t2=2*t;
        Complex Zetar=Zetaqr[t2];
        Complex Zetar1=Zetaqr[t2+1];
        for(size_t s=0; s < m; ++s) {
          f2t[s] += realproduct(Zetar,Ft[s]);
          f2t1[s] += realproduct(Zetar1,Ftm[s]);
        }
      });
  }
