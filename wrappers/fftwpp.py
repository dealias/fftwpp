...
#     """Implicitly zero-padded Hermitian convolution class.
...
#     >>> f = np.fft.rfftn(np.sin(x))
#     >>> g = np.fft.rfftn(np.sin(5*x))

#     At this point, both ``f`` and ``g`` have shape ``(N/2+1)``:
...
#     >>> np.allclose(np.fft.irfft(f)/32, np.sin(x)*np.sin(5*x))
#     True
...
#     >>> f = np.fft.rfftn(np.sin(x))
#     >>> g = np.fft.rfftn(np.sin(5*y))

#     At this point, both ``f`` and ``g`` have shape ``(N, N/2+1)``.

#     >>> assert f.shape == (N, N/2+1)
#     >>> assert g.shape == (N, N/2+1)
...
#     # >>> np.allclose(np.fft.irfftn(f)/32**2, np.sin(x)*np.sin(5*y))
#     # True
...
#     >>> f = np.fft.rfftn(np.sin(x)*np.sin(z))
#     >>> g = np.fft.rfftn(np.sin(5*y)*np.sin(2*z))

#     At this point, both ``f`` and ``g`` have shape ``(N, N, N/2+1)``.

#     >>> assert f.shape == (N, N, N/2+1)
#     >>> assert g.shape == (N, N, N/2+1)
