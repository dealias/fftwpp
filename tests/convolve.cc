...
//    if(m >  M) M=m;
  ...
  if(q == 1) {
    ...
    initZetaqm(q,centered && p % 2 == 0 ? m+1 : m);
  }
}
  ...
  if(r == 0) {
    ...
    if(2*n2 < n) { // n odd, r=0
      ...
      for(unsigned int s=mpH; s < m; ++s)
        W[s]=fmpH[s]+fH[s];
      Complex *Zetaqn2=Zetaqp+p2s1*n2;
      ...
      unsigned int m1n=m1*n;
      unsigned int m1n2=m1*n2;
      Complex *Zetan2=Zetaqm+m1n2;
      for(unsigned int s=1; s < m; ++s)
        V[s] *= Zetan2[s];
      for(unsigned int u=1; u < p2-1; ++u) {
        unsigned int um=u*m;
        Complex *Wu=W+um;
        Complex *Vu=V+um;
        Complex *Zeta0=Zetaqm+m1n*u;
        Complex *Zetan2=Zeta0+m1n2;
        for(unsigned int s=1; s < m; ++s){
          Wu[s] *= Zeta0[s];
          Vu[s] *= Zetan2[s];
        }
    ...
    for(unsigned int t=1; t < p2-1; ++t) {
      ...
      Complex Zeta=Zetaqr[t];
      for(unsigned int s=0; s < m; ++s) {
        Wt[s]=Zeta*fmpHt[s]; //*zeta_q^{tr}
        Vt[s]=Zeta*fHt[s]; //*zeta_q^{tr}
      }
    }
    Complex *Wt=W+p2s1m;
    Complex *Vt=V+p2s1m;
    ...
    fftp->fft(W);
    fftp->fft(V);
    unsigned int p2m1=p2*m1;
    for(unsigned int u=0; u < p2; ++u) {
      Complex *Zetar=Zetaqm+m1*(u*n+r);
      Complex *Zetarm=Zetar+p2m1;
      unsigned int um=u*m;
      Complex *Wu=W+um;
      Complex *Vu=V+um;
      Wu[0]=Vu[0]=Wu[0]+Vu[0];
      for(unsigned int s=1; s < m; ++s) {
        ...
        Vec Wus=LOAD(Wu+s);
        Vec Vus=LOAD(Vu+s);
        Vec A=Zetam*UNPACKL(Wus,Wus)+Zeta*UNPACKL(Vus,Vus);
        Vec B=ZMULTI(Zetam*UNPACKH(Wus,Wus)+Zeta*UNPACKH(Vus,Vus));
        STORE(Wu+s,A+B);
        STORE(Vu+s,CONJ(A-B));
      }
    }
...
void fftPadCentered::backwardInner(Complex *F0, Complex *f, unsigned int r0, Complex *W, double)
{
  ...
  if(r0 == 0) {
    ...
    if(2*n2 < n) { //n odd
      ...
      Complex *Zetaqr=Zetaqp+p2s1*r0; //FIX ME
      for(unsigned int t=1; t < p2-1; ++t) {
        unsigned int tm=t*m;
        Complex *Wt=W+tm;
        Complex *Vt=V+tm;
