                                        ...
                                        Complex *f1c)
{
  ...
  if(even && m > 2) {
    ...
    for(unsigned int b=0; b < B; ++b) {
      ...
      Vec F2=ZMULT(zeta1,LOAD(c2[b]+1));
      Vec S=F1+F2;
    ...
    for(unsigned int K=0; K <= D; K += s) {
      ...
      for(unsigned int b=0; b < B; ++b) {
        ...
        for(unsigned int k=max(even+1,K); k < stop; ++k) {
          ...
          Vec F2=ZMULT(zetak,LOAD(c2[b]+k));
          Vec S=F1+F2;
          F2=CONJ(F0+Mhalf*S)-HSqrt3*FLIP(F1-F2);
  ...
  if(d == D+1) {
    ...
    if(d == 1 && even) {
      for(unsigned int b=0; b < B; ++b) {
        ...
        Vec F2=ZMULT(Zetak,LOAD(c2[b]+d));
        Vec S=F1+F2;
        STORE(f0+d,F0+S);
                                    ...
                                    unsigned int i, unsigned int offset)
{
  ...
  if(outofplace) {
    ...
    for(unsigned int b=0; b < B; ++b) {
      ...
      rc->fft(c1b);
      double R=c1b[0].re;
      c0b[start]=S[b]; // r=0, k=c-1 (c) for m=even (odd)
      if(even) {
        Complex tmp=c1c[b];
        c1c[b]=c1b[1]; // r=1, k=1
        c1b[1]=tmp;    // r=0, k=c
