...
int main(int argc, char* argv[])
{
  ...
  if(group.rank < group.size) {
    ...
    if(main) {
      fftPadReal fftx(Lx,Mx,appx,d.y);
      P.x.init(&fftx);
    ...
    for(size_t a=0; a < A; ++a) {
      ...
      for(size_t i=0; i < Lx; ++i) {
        for(size_t j=0; j < d.y; ++j) {
          ...
          fa[d.y*i+j]=Output || testError ? i+a*J+1 : 0.0;
        }
      }
    }
    ...
    if(Output || testError) {
      ...
      for(unsigned int a=0; a < A; ++a) {
        flocal[a]=doubleAlign(Lx*Ly);
        gathery(f[a],flocal[a],d,1,group.active);
        if(main && Output)  {
          cout << "\nGathered input " << a << ":" << endl;
          Array2<double> flocala(Lx,Ly,flocal[a]);
          cout << flocala << endl;
        }
      }
      ...
      double *foutgather=doubleAlign(Lx*Ly);
      ...
      if(main) {
        ...
        if(Output) {
          ...
          Array2<double> flocal0(Lx,Ly,flocal[0]);
          cout << flocal0 << endl;
        }
